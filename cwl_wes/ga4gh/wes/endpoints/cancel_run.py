...
def cancel_run(
    config: Config, celery_app: Celery, run_id: str, *args, **kwargs
) -> Dict:
    """Cancels running workflow."""
    foca_config = config.foca
    collection_runs: Collection = (
        foca_config.db.dbs["cwl-wes-db"].collections["runs"].client
    )
    ...
    if document["api"]["state"] in States.CANCELABLE:
        ...
        logger.info(
            (
                "Canceling run '{run_id}' as background task " "'{task_id}'..."
            ).format(
                run_id=run_id,
                task_id=task_id,
            )
